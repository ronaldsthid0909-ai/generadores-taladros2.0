...
st.markdown(
    ...
    <style>
        ...
        /* Contraste consistente en controles y tablas */
...
def calculate_metrics(df, gen_cols, total_load_col, total_power_col):
    ...
    # Algunos archivos traen la columna total vacía o con ceros. En ese caso se
    # calcula el promedio real de las cargas activas para evitar mostrar 0.00%.
...
def make_fleet_chart(summary):
    ...
    s = s.sort_values("Exposición (%)", ascending=True)
    ...
    fig = go.Figure(
        go.Bar(
            ...
            y=s["Taladro"],
            orientation="h",
            text=s["Exposición (%)"].map(lambda x: f"{x:.1f}%"),
    ...
    fig.update_layout(
        ...
        title=dict(text="Exposición a baja carga simultánea por taladro", font=dict(size=14, color="#ffffff")),
        xaxis=dict(title="Porcentaje del período analizado (%)", color="#ffffff", gridcolor="rgba(255,255,255,.08)"),
...
for rig, raw_df in rig_data.items():
    try:
        ...
        interval_h = df[time_col].diff().dt.total_seconds().median() / 3600 if len(df) > 1 else 0
        interval_h = float(interval_h) if pd.notna(interval_h) and interval_h > 0 else 0.0
        analyzed_h = interval_h * len(df)
        event_h = float(events["Duración (h)"].sum()) if not events.empty else 0.0
        exposure = (event_h / analyzed_h * 100.0) if analyzed_h else 0.0
        avg_active = float(work["generadores_activos"].mean()) if len(work) else 0.0
        ...
        all_results.append({
            ...
            "Tiempo analizado (h)": round(analyzed_h, 2),
            "Exposición (%)": round(exposure, 2),
            "GEN activos prom.": round(avg_active, 2),
        })
    except Exception as e:
...
interval_h = res["df"][res["time_col"]].diff().dt.total_seconds().median() / 3600 if len(res["df"]) > 1 else 0
interval_h = float(interval_h) if pd.notna(interval_h) and interval_h > 0 else 0.0
analyzed_h = interval_h * len(res["df"])
event_h = float(res["events"]["Duración (h)"].sum()) if not res["events"].empty else 0.0
exposure_pct = event_h / analyzed_h * 100 if analyzed_h else 0.0
avg_active = float(res["work"]["generadores_activos"].mean()) if len(res["work"]) else 0.0
...
# Resumen ejecutivo: primero responde si existe una oportunidad operacional.
executive_cols = st.columns(6)
executive_data = [
    ("POTENCIA PROMEDIO TOTAL", f"{res['overall_power']:.2f} kW" if res["overall_power"] else "N/D", "demanda media"),
    ("GEN ACTIVOS PROMEDIO", f"{avg_active:.2f}", f"{len(res['gen_cols'])} disponibles"),
    ("TIEMPO ANALIZADO", f"{analyzed_h:.1f} h", period_str),
    ("HORAS EN CONDICIÓN", f"{event_h:.2f} h", "baja carga simultánea"),
    ("EXPOSICIÓN", f"{exposure_pct:.1f}%", "del período analizado"),
    ("OPORTUNIDADES", f"{len(res['events'])}", f"> {min_hours:g} h continuas"),
]
for col, (label, value, sub) in zip(executive_cols, executive_data):
    with col:
        render_card(label, value, "#00A8E8", sub)

st.markdown('<div class="section-title">CARGA PROMEDIO POR GENERADOR</div>', unsafe_allow_html=True)
...
for i in range(1, 5):
    ...
    txt = f"{val:.2f}%" if val is not None else "N/A"
    with cols[i - 1]:
        render_card(f"Carga promedio GEN {i}", txt, accents[i - 1], "cuando está activo")
...
with c_right:
    ...
    with r1_1:
        render_card(f"OPORTUNIDADES >{min_hours:g} H", f"{len(ev_df)}", "#00A8E8", f"{tot_h:.2f} h acumuladas")
    with r1_2:
...
# Tablas y Resultados
st.markdown('<div class="section-title">OPORTUNIDADES DE OPTIMIZACIÓN DETECTADAS</div>', unsafe_allow_html=True)
if res["events"].empty:
...
fc1, fc2 = st.columns([1.0, 1.35], gap="medium")
with fc1:
    st.plotly_chart(make_fleet_chart(summary), use_container_width=True, config={"displaylogo": False})
with fc2:
    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
    fleet_cols = ["Taladro", "Tiempo analizado (h)", "Horas en eventos >5h", "Exposición (%)", "Eventos >5h", "Mayor evento (h)", "GEN activos prom."]
    st.dataframe(summary[fleet_cols].sort_values("Exposición (%)", ascending=False), use_container_width=True, hide_index=True, height=340)
